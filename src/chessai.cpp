...
int ChessAI::calcOnePosGreedy(int (*board)[15], int row, int col, int C_ME)
{
    int sum = 0;
    for (int i = 0; i < 4; ++i)
    {                               //四个方向
        for (int j = 0; j < 5; ++j) //每个方向上最多5个五元组
        {
            ...
            { //若五元组下标均合法
                ...
                { //对五元组5个位置考察
                    ...
                    if (board[tmp.x()][tmp.y()] == C_BLACK)
                        blackChess++;
                    if (board[tmp.x()][tmp.y()] == C_WHITE)
                        whiteChess++;
                }
...
QPoint ChessAI::findBestMoveGreedy(int C_ME, int (*board)[15])
{
    int bestScore = 0;
    ...
    for (int i = 0; i < 15; ++i)
    {
        for (int j = 0; j < 15; ++j)
        {
            if (board[i][j] == C_NONE)
            { //空的位置
                int score = calcOnePosGreedy(board, i, j, C_ME);
                if (bestScore < score)
                {
                    bestScore = score;
                    bestRow = i;
...
POINTS ChessAI::seekPoints(int (*board)[15])
{
    bool B[15][15]; //标记数组
    int worth[15][15];
    POINTS bestPoints;
